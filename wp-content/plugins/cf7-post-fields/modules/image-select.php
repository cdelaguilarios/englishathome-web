...
/*
 * TODO
 * Überdenken der Image size und excerpt
 * CF7 Update shortcode_handler Kontrolle
 */
...
if(!class_exists('wpcf7_post_fields_image_select') )
{
    class wpcf7_post_fields_image_select extends wpcf7_post_fields_module
    {
        ...
        {
            add_image_size('wpcf7-post-image-select', 80, 80, true);
        ...
        public function wpcf7_post_image_select_shortcode_handler( $tag )
        {
            ...
            foreach ( $values as $key => $value )
            {
                ...
                if(get_post_status($ids[$key]) !== false)
                {
                    ...
                    $image_url = has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, $image_size) : '';
         ...
         */
        private function sanitize_image_size($image_size, $default = 'wpcf7-post-image-select')
        {
            if($image_size === false) {
                return $default;
            }
         ...
         */
        private function get_image_width($size, $default = 80)
        {
